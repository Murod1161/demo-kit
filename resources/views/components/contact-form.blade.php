...
<form @class([
    'text-white' => $isLight,
    'text-secondary' => !$isLight,
    'feedback xl:w-222 2xl:w-258 grid gap-y-10 sm:grid-cols-2 sm:gap-x-10 md:gap-x-20 md:gap-y-10 xl:mx-auto',
]) method="POST" action="{{ route('contact.submit') }}" enctype="multipart/form-data" autocomplete="off">

    @csrf

    <!-- Тема обращения -->
    <div class="relative sm:col-span-1">
        <select id="themeSelect" @class([
            'text-white' => $isLight,
            'text-secondary' => !$isLight,
            'peer w-full appearance-none border-0 border-b bg-transparent pb-2 pr-8 pt-6 font-futura focus:outline-none',
        ]) required>
            <option value="" disabled selected hidden></option>
            <option class="bg-primary p-10 text-xl text-secondary" value="client">Я потенциальный клиент</option>
    ...
    <div class="flex flex-col items-start justify-between gap-7 sm:col-span-2 md:flex-row-reverse">
        ...
        <div>
            ...
            <!-- Имя файла -->
            <p class="mt-1 hidden text-xs italic text-gray-300" id="file-name">Файл не выбран</p>
        ...
        <div class="flex w-full flex-row-reverse items-center justify-between gap-8 md:w-auto md:flex-row lg:gap-10">
            ...
            <p @class([
                ...
                'text-secondary/50' => !$isLight,
                'max-w-70 xl:max-w-100 m-0 text-xs leading-none md:text-base',
            ])>
